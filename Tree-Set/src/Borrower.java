...
public class Borrower {
    ...
    public void getBookBenchmark(String title) throws IOException {
        Book dummyBook = new AudioBook(title, "", "", 0, ""); // Create a dummy Book object with the user input title

        if (BookInterface.library.contains(dummyBook)) { // Check if the dummyBook title matches a title in the TreeSet
    ...
    public void getBook(String title) throws IOException {
        ...
        if (BookInterface.library.contains(dummyBook)) { // Check if the dummyBook title matches a title in the TreeSet
            Book foundBook = BookInterface.library.floor(dummyBook); // Retrieve the actual book using floor method
